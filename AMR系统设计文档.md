...
> 文档定位：本文件保留产品背景、早期方案和扩展能力说明。当前模块边界以 `系统架构.md` 为准，页面字段、交互和验收以 `前端UI需求文档.md` 为准。
...
| 派单管理 | 实时任务、任务配置、调度策略、调度记录 | 任务详情、新建/编辑任务配置 |
| 交通管制 | 交通态势、管制资源、管制记录 | 资源详情、记录详情 |
| 行为树管理 | 行为树列表、行为树编辑器 | 版本详情 |
| 地图管理 | 地图管理、地图日志 | 地图编辑器、地图版本详情、扫描建图流程 |
| 资源管理 | AMR列表、设备列表、型号配置、设备类型 | AMR详情、设备详情 |
...
- 交通态势、交通资源和交通记录。
- 派单任务的默认筛选范围。
- 地图管理默认筛选和运行版本标识；地图编辑器由用户点击的地图卡片确定，不随全局范围自动切换。
- AMR和设备列表的默认筛选范围。
- 告警中心的默认告警范围。

地图编辑器绑定当前打开的地图，不提供编辑器内地图切换。离开存在未保存修改的编辑器时，应提示用户保存草稿、放弃修改或取消离开。

## 7. 主要功能设计

### 7.1 首页

首页不再拆分二级页面，并单独隐藏二级侧边栏。中央使用厂内物流3D数字孪生静态效果图，两侧保留AMR、任务、交通和异常看板，不设置顶部指标看板或快捷操作。

主要内容：

- 展示厂内产线、机台、仓储区域和AMR的3D孪生静态效果。
- 左侧展示AMR利用率、电量分布和车辆状态。
- 右侧展示当前任务、交通资源和运行异常。
- 楼层和地图继续由顶部全局运行范围控制。

数字孪生能力成熟后，再以真实孪生引擎替换当前静态效果图。
...
实时展示设备请求生成的任务实例，重点呈现请求来源、任务路线、当前执行阶段、执行 AMR、运行时长及异常信息，帮助现场人员快速掌握任务执行情况。任务详情由列表进入三级页面。当前阶段不提供人工创建或补单入口，任务实例统一由设备或 API 请求触发。

#### 7.3.2 任务配置

维护设备请求与标准物流任务的匹配关系，包括设备触发条件、任务类型、默认路线、优先级、调度策略和行为树。新建与编辑配置均由任务配置列表进入三级页面，不单独占用二级菜单。
...
地图管理卡片库是地图模块默认首页。用户点击指定地图卡片后进入“地图管理 / 地图名称 / 地图编辑器”三级页面。编辑器绑定所点击的地图，不在编辑器内部切换其他地图；进入后切换为全屏工程工作台，隐藏普通内容页标题和二级导航，将空间优先分配给中央地图画布。
...
地图管理页面只负责地图档案及生命周期，不直接编辑道路和点位。页面使用以地图缩略图为主体的卡片库，不承担运行指标看板职责。

主要功能：

- 通过创建向导建立地图，并在向导中选择扫描底图、适用 AMR 和现场设备。
- 通过地图缩略图卡片查看地图编号、名称、专案、楼层、底图来源、资源范围、逻辑对象、状态和更新人。
- 查看运行版本和编辑版本。
- 点击地图卡片进入该地图的三级地图编辑器；通过卡片资料设置按钮编辑基础信息、更换扫描底图、调整适用 AMR 与现场设备。
- 复制、停用、归档和查看历史版本。
- 创建从未发布且没有业务关联的空地图。

地图管理下新增“地图日志”二级目录，记录创建地图、资料修改、扫描底图更换、逻辑地图编辑、校验、发布、下发和归档操作。日志至少保留操作时间、操作类型、地图、版本、操作人、变更内容和操作结果，并支持条件查询。
...
AMR 运行时的地图相似度比对属于 AMR 内部能力，平台地图管理和地图编辑器不展示相似度、差异区域或比对参数。平台只管理 AMR 扫描底图、逻辑地图、资源定位、版本发布与地图下发。
...
详情页提供返回 AMR 列表和地图定位操作，不提供人工创建指定任务入口。
